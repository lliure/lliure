...
$where = '';
if (isset($_GET['idCat'])) {
	$where .= " and b.idCat = '".$_GET['idCat']."'";
}
$pluginTable = SUFIXO."catalogo";

if (isset($_POST['del']) && is_array($_POST['del'])) {
	$del = "DELETE FROM ".$pluginTable."_relacao where idProd in (".in($_POST['del']).")";
	mysql_query($del);
...
LEFT JOIN sgs_catalogo_relacao b
on a.id = b.idProd
where 1=1 $where
".$montaOrdem;
...
<?php
	...
	<table class="table">
		<tr>
			<th><a href="<?=$pluginHome?>&amp;acoes=produtos&amp;ordem=nome<?=$mudaLinkOrdem?>" title="Ordernar por nome"><img src="imagens/icones/order.gif" alt="Ordernar por nome"/></a> Imóveis</th>		
	...
	?>
		...
		echo mensagemAviso('Nenhum produto encontrado');
	}
?>
</div>